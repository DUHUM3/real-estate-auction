...
const ClientsSlider = ({ clients, onClientClick }) => {
  ...
  const getVisibleLogos = () => {
    ...
    const maxVisibleLogos = isMobile ? 1 : Math.min(3, clients.length);
    ...
    for (let i = 0; i < maxVisibleLogos; i++) {
      ...
      visibleLogos.push({
        ...clients[index],
        isActive: isMobile ? true : i === Math.floor(maxVisibleLogos / 2),
      });
    }
            ...
            <>
              ...
              <div className="block md:hidden">
                ...
                <div className="flex items-center justify-center my-6 sm:my-8">
                  <div className="bg-white rounded-xl p-4 w-full max-w-xs shadow-sm">
                    <div className="h-32 sm:h-40 flex items-center justify-center mb-3">
                      <img
                  ...
                  <div className="flex items-center justify-center gap-4 md:gap-8 transition-all duration-500">
                    {getVisibleLogos().map((client) => (
                      <div
                        ...
                        style={{ flex: "0 0 33.33%", height: "120px" }}
        ...
        <div className="hidden md:flex items-center justify-center my-10 gap-6">
          ...
          <div className="flex gap-8">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="w-36 h-20 bg-gray-300 rounded-lg animate-pulse"
              ></div>
            ))}
          </div>
          <div className="w-12 h-12 bg-gray-300 rounded-full animate-pulse"></div>
...
const fetchClients = async () => {
  const response = await fetch(
    "https://core-api-x41.shaheenplus.sa/api/clients/featured"
  );
