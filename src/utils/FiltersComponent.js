...
const FiltersComponent = memo(
  ({
    ...
    const availableRegions = useMemo(
      () => saudiRegions.map((r) => r.name.trim()),
      []
    );
    ...
    const availableCities = useMemo(() => {
      ...
      const region = saudiRegions.find((r) => r.name.trim() === filters.region);
  ...
  <>
    <div className="mb-4 md:mb-0 md:w-1/4 px-2">
      ...
      <select
        ...
        className="w-full px-3 py-2 border rounded-lg rtl text-right bg-white"
      >
