...
function Register({ onClose, onSwitchToLogin }) {
  ...
  // إضافة حالة لفحص قوة كلمة المرور
  ...
  const calculatePasswordStrength = useMemo(() => {
    return (password) => {
      if (!password) return { score: 0, feedback: "أدخل كلمة المرور" };
      ...
      return {
        ...
        strength,
        isStrong: score >= 4,
        isValidForRegistration: score >= 3, // تعديل: 3/5 كحد أدنى للتسجيل
      };
    };
  }, []);
  ...
  // Validate current step inputs - مع فحص قوة كلمة المرور فقط في الخطوة الأخيرة
  const validateCurrentStep = () => {
    let errors = {};
    let isValid = true;

    if (currentStep === 1) {
      // Validate personal info فقط
      if (!formData.full_name.trim()) {
        errors.full_name = "الرجاء إدخال الاسم الكامل";
        isValid = false;
      } else if (formData.full_name.trim().split(" ").length < 2) {
        errors.full_name =
          "الرجاء إدخال الاسم الكامل (الاسم الأول والاسم الأخير على الأقل)";
        isValid = false;
      }
      ...
      // Validate business/license info based on user type فقط
      if (userTypeId === 4) {
      ...
      currentStep === 3 ||
      (currentStep === 2 && ![4, 5, 6].includes(userTypeId))
    ) {
      // هنا فقط نتحقق من قوة كلمة المرور لأنها الخطوة الخاصة بها
  ...
  // Navigation functions - تعديل: السماح بالتنقل بين الخطوات دون فحص قوة كلمة المرور
  const nextStep = () => {
    // التحقق من صحة الخطوة الحالية فقط
  ...
  const handleSubmit = async (e) => {
    ...
    // إذا لم نكن في الخطوة الأخيرة، انتقل للخطوة التالية فقط
    if (currentStep < maxSteps) {
      nextStep();
      return;
    }

    // فقط في الخطوة الأخيرة نتحقق من قوة كلمة المرور ونرسل النموذج
    if (!validateCurrentStep()) return;

    // منع تسجيل الدخول بكلمة مرور ضعيفة
    ...
    try {
      ...
      if (
        userTypeId === 3 &&
        formData.agency_number &&
        formData.agency_number.trim()
      ) {
      ...
      if (userTypeId === 4) {
        ...
        if (
          formData.commercial_register &&
          formData.commercial_register.trim()
        ) {
          formDataToSend.append(
            "commercial_register",
            formData.commercial_register.trim()
      ...
      if (errorMsg.includes("email") || errorMsg.includes("بريد")) {
        ...
        errorMsg.includes("national_id") ||
        errorMsg.includes("هوية")
      ) {
        setFieldErrors({
          ...fieldErrors,
        ...
        errorMsg.includes("commercial_register") ||
        errorMsg.includes("سجل")
      ) {
        setFieldErrors({
          ...fieldErrors,
          commercial_register: "رقم السجل التجاري مستخدم بالفعل",
        });
        setCurrentStep(2);
      } else if (
        errorMsg.includes("license_number") ||
        errorMsg.includes("ترخيص")
      ) {
        setFieldErrors({
          ...fieldErrors,
          license_number: "رقم الترخيص مستخدم بالفعل",
        });
        setCurrentStep(2);
      } else if (errorMsg.includes("full_name") || errorMsg.includes("اسم")) {
        setFieldErrors({
          ...fieldErrors,
          full_name: "الرجاء إدخال الاسم الكامل",
        });
        setCurrentStep(1);
      } else {
...
const handleVerifyEmail = async (e) => {
    ...
    if (response.verified) {
      ...
      setTimeout(async () => {
        try {
          ...
          // إذا فشل تسجيل الدخول → الانتقال لشاشة تسجيل الدخول
          setTimeout(() => {
            onSwitchToLogin();
          }, 1500);
        }
  ...
  // مكون عرض قوة كلمة المرور - يظهر فقط في الخطوة الأخيرة
      ...
      <div className="mt-2 sm:mt-3">
        ...
        <div className="h-1.5 sm:h-2 bg-gray-200 rounded-full overflow-hidden mb-1">
          <div
            ...
            style={{
              width: `${
                (passwordStrength.score / passwordStrength.total) * 100
              }%`,
            }}
          ></div>
        </div>
      ...
      return (
        <div className="space-y-3 sm:space-y-4 text-center">
          ...
          <p className="text-xs text-gray-500 mt-2">
            يمكنك الآن إغلاق هذه النافذة والبدء في استخدام حسابك. شكراً لانضمامك
            إلينا!
          </p>
        ...
        {currentStep === 2 && userTypeId === 5 && (
          <>
            ...
            {renderFileUploadField(
              "license_file",
              "ملف الترخيص العقاري",
              "الرجاء رفع ملف الترخيص العقاري"
            )}
          </>
        )}
